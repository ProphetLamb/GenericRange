...
So instead of multiple implementation of the same thing, but with a different underlying `Type` this project attempts to provide a API simmilar to the existing, but with whatever primitive the user desires.
...
Intstall the [NuGet package](https://www.nuget.org/packages/GenericRange/)

```bash
dotnet add package GenericRange --version 0.0.8
```
...
```csharp
Range<Foo> fooRange = (Foo.One, Foo.Eight); // Tmplicit cast from ValueTuple<T,T> to Range<T>
...
Read the [autogenerated](https://github.com/lijunle/Vsxmd) doc file [here](./src/doc.md).